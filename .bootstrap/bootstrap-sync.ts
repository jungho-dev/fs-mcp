...
const getRuntimeConfig = (config=env) => {
  ...
  const result = {
    remoteEntryRelativePath: String(runtime.remoteEntryRelativePath || `.lib/public-router.ts`).trim(),
...
const getMessageConfig = (config=env) => {
  ...
  const result = {
    ...
    remoteNotFound: String(messages.remoteNotFound || `원격 public/.lib/public-router.ts를 찾지 못했습니다.`),
  };
