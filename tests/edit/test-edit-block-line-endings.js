...
function assertEditBlockSuccess(result, message) {
  ...
  assert.ok(/\[R\] Reading \d+ lines? from/.test(result.content[0].text), `${message} (text should contain file-preview status line)`);
}
...
async function testLargeFilePerformance() {
  ...
  try {
    ...
    const lines = new Array(800).fill("This is a line in a large file.\n"); // Reduced from 2000 to 800
...
async function testEdgeCases() {
  ...
  try {
    ...
    assert.ok(result.content[0].text.includes("Search content not found"), "Should handle empty file correctly");
