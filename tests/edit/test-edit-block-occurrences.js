...
import { handleEditBlock } from "../../out/mcp/controllers/edit-controller.mjs";
...
async function testMoreOccurrencesThanExpected() {
  try {
    ...
    assert.ok(result.content[0].text.includes("Expected 1 occurrences but found 4"), "Should report the correct number of occurrences");
  } catch (error) {
...
async function testFewerOccurrencesThanExpected() {
  try {
    ...
    // Check that we got an error about the number of occurrences
    assert.strictEqual(result.content[0].type, "text", "Result should be text");
    assert.ok(result.content[0].text.includes("Expected 3 occurrences but found 1"), "Should report the correct number of occurrences");
  } catch (error) {
    console.error("Test failed:", error);
    throw error;
  }
...
async function testExactNumberOfOccurrences() {
  try {
    ...
    assert.strictEqual(result.content[0].type, "text", "Result should be text");
    assert.ok(result.content[0].text.includes("Successfully applied 4 edits"), "Should report success with the correct number of edits");

    // Verify the file content
    const fileContent = await fs.readFile(MULTI_OCCURRENCE_FILE, "utf8");
    const expectedContent = `This line has been replaced correctly.
This is a unique line.
...
This is a MODIFIED target line in the header.`,
    ...
    // Check that the operation succeeded
    assert.strictEqual(result.content[0].type, "text", "Result should be text");
    assert.ok(result.content[0].text.includes("Successfully applied 1 edit"), "Should report success with the header edit");
...
This is a MODIFIED target line in the footer.`,
    ...
    // Check that the operation succeeded
    assert.strictEqual(result.content[0].type, "text", "Result should be text");
    assert.ok(result.content[0].text.includes("Successfully applied 1 edit"), "Should report success with the footer edit");

    // Verify the file content
    const fileContent = await fs.readFile(CONTEXT_TEST_FILE, "utf8");
    const expectedContent = `Header section
This is a MODIFIED target line in the header.
End of header section
...
async function testNonExistentPattern() {
  try {
    ...
    assert.strictEqual(result.content[0].type, "text", "Result should be text");
    assert.ok(result.content[0].text.includes("Search content not found"), "Should report that the search content was not found");
  } catch (error) {
    console.error("Test failed:", error);
    throw error;
  }
...
async function testEmptySearchString() {
  try {
    ...
    assert.strictEqual(result.content[0].type, "text", "Result should be text");
    assert.ok(result.content[0].text.includes("Empty search strings are not allowed"), "Should report that empty search strings are not allowed");
  } catch (error) {
    console.error("Test failed:", error);
    throw error;
  }
