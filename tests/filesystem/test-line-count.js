...
async function testLineCount() {
  ...
  {
    const filePath = await createTestFile("no-trailing.txt", "line1\nline2\nline3");
    const result = await handleReadFile({ path: filePath, offset: 0, length: 2 });
    const total = extractTotalLines(result);
    try {
  ...
  {
    const filePath = await createTestFile("single-trailing.txt", "hello\n");
  ...
  {
    const filePath = await createTestFile("single-no-trailing.txt", "hello");
    const result = await handleReadFile({ path: filePath, offset: 0, length: 1000 });
    const total = extractTotalLines(result);
    try {
  ...
  {
    const lines = Array.from({ length: 100 }, (_, i) => `Line ${i + 1}`);
    const filePath = await createTestFile("hundred-no-trail.txt", lines.join("\n"));
