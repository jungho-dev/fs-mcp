...
async function testNodeREPL() {
  ...
  const debugFile = path.join(OUTPUT_DIR, "node-repl-debug.txt");
  let debugLog = "";
  ...
  node.stdout.on("data", (data) => {
    ...
    outputBuffer += text;
    log(`${colors.green}${text.trim()}${colors.reset}`);
  });

  node.stderr.on("data", (data) => {
    const text = data.toString();
    outputBuffer += text;
    log(`${colors.red}${text.trim()}${colors.reset}`);
  });
...
`;
  ...
  const containsGreetings = outputBuffer.includes("Hello, User 0!") && outputBuffer.includes("Hello, User 1!") && outputBuffer.includes("Hello, User 2!");
