...
import { handleGetMoreSearchResults, handleStartSearch, handleStopSearch } from "../../out/mcp/controllers/search-controller.mjs";
...
async function setupEdgeCases() {
  ...
  const longLine = `${"a".repeat(10_000)}LONGLINEPATTERN${"b".repeat(10_000)}`;
  await fs.writeFile(path.join(EDGE_CASE_TEST_DIR, "long-lines.txt"), longLine);

  // File with special characters
  await fs.writeFile(path.join(EDGE_CASE_TEST_DIR, "special-chars.txt"), "Special chars: @#$%^&*(){}[]|\\:\";'<>?,./\nUnicode: :)***●\nPattern with special chars: test@pattern");
  ...
  await fs.writeFile(path.join(EDGE_CASE_TEST_DIR, "regex-chars.txt"), "Content with regex chars: .+*?^${}()|[]\\\nPattern: test.pattern\nAnother: test*pattern");
...
async function teardownEdgeCases(originalConfig) {
  ...
  try {
    const { handleListSearches, handleStopSearch } = await import("../../out/mcp/controllers/search-controller.mjs");
...
async function testLargeFiles() {
  ...
  assert(duration < 10_000, `Search should complete within 10 seconds, took ${duration}ms`);
}
...
async function testManySmallFiles() {
  ...
  try {
    ...
    const promises = [];
    for (let i = 0; i < 100; i++) {
      promises.push(fs.writeFile(path.join(manyFilesDir, `file${i}.txt`), `This is file ${i} with pattern ${i}`));
    }
...
export async function testSearchCodeEdgeCases() {
  ...
  try {
    ...
    try {
      const { searchManager, stopSearchManagerCleanup } = await import("../../out/features/search/search-service.mjs");
