...
async function setup() {
  ...
  await fs.writeFile(
    path.join(LITERAL_SEARCH_TEST_DIR, "code-patterns.js"),
    `// JavaScript file with common code patterns
...
`,
  ...
  await fs.writeFile(
    path.join(LITERAL_SEARCH_TEST_DIR, "similar-patterns.ts"),
    `// TypeScript file with similar patterns
interface Config {
  getValue(): string;
}
...
async function testOriginalFailingCase() {
  ...
  assert(text.includes('toast.error("test")') || text.includes("code-patterns.js"), "Should find the exact pattern that was originally failing");
...
async function testRegexFailureLiteralSuccess() {
  ...
  assert(literalMatches > regexMatches, `Literal search should find more matches (${literalMatches}) than regex search (${regexMatches}) for patterns with special characters`);
