...
import type { DirectoryListingEntryType, ServerResult } from '../types/index.mjs';
import { resolvePreviewFileType } from '../utils/files/preview-file-types.mjs';
import type { ReadOptions } from '../utils/files/base.mjs';
...
const DIRECTORY_LISTING_ENTRY_PATTERN = /^(?:\[(File|Dir|Warning|Denied|FILE|DIR|WARNING|DENIED)\]|(📄|📁|🟠|🔴))\s*(.*)$/;
...
function resolveDirectoryListingEntryType(match: RegExpMatchArray | null): DirectoryListingEntryType {
    ...
    if (bracketType === 'file' || bracketType === 'dir' || bracketType === 'warning' || bracketType === 'denied') {
        return bracketType;
    }
