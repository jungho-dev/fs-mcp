/**
 * @file src/features/history/tool-history-store.mts
 * @description Tool call history store.
 * @author JUNGHO
 * @since 2026-05-02
 */

import type { Stats } from "node:fs";
import { existsSync, promises as fs, mkdirSync, readFileSync, statSync, writeFileSync } from "node:fs";
...
class ToolHistory {
  ...
  constructor() {
    ...
    const historyDir = "C:/Users/jungh/.codex/mcp/fs-mcp";
  ...
  private loadFromDisk(): void {
    try {
      ...
      this.trimHistoryFileIfTooLargeSync();
   ...
   */
  private async trimHistoryFileIfTooLarge(): Promise<boolean> {
    let stats: Stats;
    try {
      stats = await fs.stat(this.historyFile);
    ...
    try {
      ...
      await fs.writeFile(this.historyFile, `${kept.join("\n")}\n`, "utf-8");
      return true;
    } catch (_error) {
  ...
  private trimHistoryFileIfTooLargeSync(): boolean {
    let stats: Stats;
    try {
      if (!existsSync(this.historyFile)) {
        return false;
      }
      stats = statSync(this.historyFile);
      if (stats.size <= this.MAX_HISTORY_FILE_SIZE_BYTES) {
        return false;
      }
    } catch (_error) {
    ...
    try {
      ...
      writeFileSync(this.historyFile, `${kept.join("\n")}\n`, "utf-8");
      return true;
    } catch (_error) {
      return false;
    }
  }
  private trimHistoryFileSync(): void {
    try {
      ...
      writeFileSync(this.historyFile, lines, "utf-8");
    } catch (_error) {
      // Silently fail
    }
  }
   ...
   */
  addCall(toolName: string, args: unknown, output: ServerResult, duration?: number): void {
   ...
   */
  getRecentCalls(options: { maxResults?: number; toolName?: string; since?: string }): ToolCallRecord[] {
   ...
   */
  getRecentCallsFormatted(options: { maxResults?: number; toolName?: string; since?: string }): FormattedToolCallRecord[] {
  ...
  getStats() {
    return {
      ...
      newestEntry: this.history.at(-1)?.timestamp,
