/**
 * @file src/features/search/ripgrep-adapter.mts
...
const RIPGREP_NOT_FOUND_MESSAGE =
  "ripgrep binary not found. fs-mcp requires ripgrep to perform searches. " +
...
function getCommonRipgrepPaths(): string[] {
  if (process.platform === WINDOWS_PLATFORM) {
    return [
      ...
      join(homedir(), "scoop", "apps", "ripgrep", "current", "rg.exe"),
      join(homedir(), ".cargo", "bin", "rg.exe"),
    ];
  }
