...
import { readFile, writeFile, readFileInternal, validatePath } from './filesystem.mjs';
import fs from 'fs/promises';
import type { ServerResult } from '../types/index.mjs';
import { recursiveFuzzyIndexOf, getSimilarityRatio } from './fuzzySearch.mjs';
import { capture } from '../utils/capture.mjs';
import { createErrorResponse } from '../core/error-handlers.mjs';
import { EditBlockArgsSchema } from "./schemas.mjs";
import path from 'path';
import { detectLineEnding, normalizeLineEndings } from '../utils/lineEndingHandler.mjs';
import { configManager } from '../config/config-manager.mjs';
import { fuzzySearchLogger, type FuzzySearchLogEntry } from '../utils/fuzzySearchLogger.mjs';
import { resolvePreviewFileType } from '../utils/files/preview-file-types.mjs';
import { resolveAbsolutePath } from '../handlers/filesystem-handlers.mjs';
...
export async function handleEditBlock(args: unknown): Promise<ServerResult> {
    ...
    let handler: Awaited<ReturnType<typeof import('../utils/files/factory.mjs').getFileHandler>>;
    try {
        validatedPath = await validatePath(parsed.file_path);
        const { getFileHandler } = await import('../utils/files/factory.mjs');
