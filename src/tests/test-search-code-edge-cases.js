...
import { handleStartSearch, handleGetMoreSearchResults, handleStopSearch } from '../../dist/handlers/search-handlers.mjs';
import { configManager } from '../../dist/config/config-manager.mjs';
...
async function teardownEdgeCases(originalConfig) {
  ...
  try {
    const { handleListSearches, handleStopSearch } = await import('../../dist/handlers/search-handlers.mjs');
...
export async function testSearchCodeEdgeCases() {
  ...
  try {
    ...
    try {
      const { searchManager, stopSearchManagerCleanup } = await import('../../dist/tools/search-manager.mjs');
