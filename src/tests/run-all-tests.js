...
async function runTestModules() {
  ...
  try {
    ...
    // Ensure the main smoke test runs first if it exists
    const prioritizedMainTest = discoveredTests.includes('test.mjs')
      ? 'test.mjs'
      : (
        discoveredTests.includes('test.js')
          ? 'test.js'
          : null
      );

    if (prioritizedMainTest) {
